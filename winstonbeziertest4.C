...
TGraph* ConeTrace(Int_t mode, bool del) {
  ...
  for (Double_t deg = 0.; deg < 89.; deg += 0.1) {
    ...
    for (Double_t phi = 0.; phi < 30.; phi += 0.3) {
      ...
      const Int_t entries = exited->GetEntries();
      int exitedphotons[entries];
      int arrSize = sizeof(exitedphotons);
      myvector.push_back(arrSize);
  
  cout << myvector;

    
//      for (Int_t k =0; k <= exited->GetLast(); k++) {
//        exitedphotons.push_back(k);
//      }  // k
//     
//      int arrSize = sizeof(exitedphotons);
//      cout << arrSize;
...
//
//        if (std::string(history->At(n - 2)->GetName()) == "coneMirror") {
//        cout << "reflected photon";
//        }  // if
//      }    // k
