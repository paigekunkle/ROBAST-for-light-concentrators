...
void HexWinstonCone(Int_t mode = 2) {
  ...
  AGeoWinstonCone2D* coneV =
      new AGeoWinstonCone2D("coneV", kRin, kRout, kDepth);
...
//  } else 
  if (mode == 2) {
    AGeoWinstonConePoly* hexV =
        new AGeoWinstonConePoly("hexV", kRin, kRout, 6);
    TGeoCompositeShape* coneComp1 =
        new TGeoCompositeShape("coneComp1", "pgon:rot30 - hexV");
    AMirror* coneMirror = new AMirror("coneMirror", coneComp1);
    world->AddNode(coneMirror, 1);
  ...
  TGeoPgon* pgonPMT = new TGeoPgon("pgonPMT", 0, 360, 100, 2);
  ...
  graAeff->SetMarkerSize(2);
  graAeff->SetMarkerColor(kRed);
  graAeff->Draw("ap");
  can1->SaveAs("lightcone.pdf");
  can2->SaveAs("collectionefficiency.pdf");
}
