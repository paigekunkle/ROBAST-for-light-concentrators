...
void AddCamera(AOpticalComponent* opt) {
  ...
  // Make a camera box
  TGeoTube* tubeCameraBox =
      new TGeoTube("tubeCameraBox", 0, kCameraBoxD / 2., kCameraBoxH / 2.);
  double t = 1 * cm;
  TGeoTube* tubeCameraBox2 = new TGeoTube(
      "tubeCameraBox2", 0, kCameraBoxD / 2. - t, kCameraBoxH / 2. - t);

  TGeoTranslation* transZ1 =
      new TGeoTranslation("transZ1", 0, 0, kF + kCameraBoxH / 2.);
  transZ1->RegisterYourself();
  TGeoTranslation* transZ2 =
      new TGeoTranslation("transZ2", 0, 0, kF + kCameraBoxH / 2. - t - 1 * mm);
  ...
  TGeoTranslation* trans = new TGeoTranslation(
      "", 0, -kLidOffset + kCameraD / 2. * TMath::Cos(kLidOpening * d2r),
      kF + kCameraD / 2. * TMath::Sin(kLidOpening * d2r));
...
void RayTrace(AOpticsManager* manager, TCanvas* can3D) {
  ...
  for (int i = 0; i < kNdeg; i++) {
    ...
    for (Int_t k = 0; k <= focused->GetLast(); k++) {
      ...
      auto history = ray->GetNodeHistory();
      auto n = history->GetEntries();

      if (std::string(history->At(n - 2)->GetName()) == "mirror_236") {
